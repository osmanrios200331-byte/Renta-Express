...
router.post('/clientes', (req, res) => {
    const nuevo = { id: generarId(clientes), ...req.body, fechaRegistro: new Date().toLocaleString() };
    clientes.push(nuevo);
...
// 3. CRUD: VEHICULOS
// ==========================================
router.get('/vehiculos', (req, res) => res.json(vehiculos));

router.post('/vehiculos', (req, res) => {
    const nuevo = { id: generarId(vehiculos), ...req.body, fechaRegistro: new Date().toLocaleString() };
...
router.post('/rentas', (req, res) => {
    ...
    // Validar que existan cliente y vehículo
    const vIndex = vehiculos.findIndex(v => v.id === parseInt(idVehiculo));
    if (vIndex === -1) return res.status(404).json({ error: "Vehículo no existe" });
...
router.post('/pagos', (req, res) => {
    const nuevo = { id: generarId(pagos), fechaPago: new Date().toLocaleString(), ...req.body };
    pagos.push(nuevo);
    res.status(201).json(nuevo);
});
